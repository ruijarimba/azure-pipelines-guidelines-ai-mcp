...
Unit tests for `AzurePipelines.Guidelines.Rules` — individual `IRule` implementations.
...
- **Violating document** → diagnostics with correct `RuleId`, severity, message, location.
...
`RuleName_GivenViolation_ShouldReturnDiagnosticWithCorrectRuleId`
...
- Rules must **never throw** — tests must prove defensive null handling.
