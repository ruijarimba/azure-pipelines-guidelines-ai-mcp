...
steps:
  - script: |
      dotnet tool install -g adog
      adog analyze $(Build.SourcesDirectory)/azure-pipelines.yml --format compact
    displayName: 'Analyze pipeline'
```

**Audit mode with SARIF upload:**

```yaml
steps:
  - script: |
      dotnet tool install -g adog
      adog analyze $(Build.SourcesDirectory) \
        --format sarif \
        --output $(Build.ArtifactStagingDirectory)/analysis.sarif \
        --soft-fail
    displayName: 'Analyze pipelines'

  - task: PublishBuildArtifacts@1
    inputs:
      pathToPublish: $(Build.ArtifactStagingDirectory)/analysis.sarif
...
steps:
  - script: |
      ...
      adog analyze $(Build.SourcesDirectory) \
        --format junit \
        --output $(Common.TestResultsDirectory)/adog.xml \
        --soft-fail
    displayName: 'Analyze pipelines'
  ...
  - task: PublishTestResults@2
    inputs:
      ...
      testResultsFiles: '$(Common.TestResultsDirectory)/adog.xml'
      testRunTitle: 'Pipeline Guidelines'
    condition: always()
```
...
steps:
  - name: Install adog
    run: dotnet tool install -g adog

  - name: Analyze pipeline
    run: adog analyze .github/workflows/ --format compact
```
...
pipeline-analysis:
  ...
  script:
    - dotnet tool install -g adog
    - adog analyze .gitlab-ci.yml --format json --output report.json --soft-fail
  artifacts:
    reports:
