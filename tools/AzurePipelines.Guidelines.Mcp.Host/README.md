...
### Discovery response diagnostics

Set `MCP_LOG_RESPONSES=true` only when troubleshooting what this server advertises to an MCP
client. The setting is disabled by default. When enabled, it writes the following completed
JSON-RPC discovery responses to `stderr`, so they can be inspected in the MCP client console or
...
For example, in PowerShell before starting the **Debug** profile:

```powershell
$env:MCP_LOG_RESPONSES = "true"
dotnet run --project tools/AzurePipelines.Guidelines.Mcp.Host --launch-profile Debug -- --transport sse
```

The diagnostic hook runs at the SDK's outgoing JSON-RPC message boundary. This is intentional:
it records the exact result object generated by the MCP server without replacing the SDK's
attribute-based discovery handlers. It correlates only allowlisted incoming request IDs with their
successful responses. It logs no request bodies, tool results, resource reads, prompt results,
notifications, or error responses. Therefore it does not log inline YAML, file paths, tool
arguments, resource content, or prompt arguments.

Payloads are capped at 32 KiB to keep the console usable. Do not enable this setting by default in
shared or production environments: discovery metadata still identifies the installed server and
its available capabilities. Remove the environment variable or set it to `false` when finished.
